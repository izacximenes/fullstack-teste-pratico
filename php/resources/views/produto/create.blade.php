...
<div>
    ...
    <div class=" p-4 px-4 md:p-8 mb-6">
        ...
        <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3">
            ...
            <form action="{{ route('produtos.store') }}" class="lg:col-span-2" method="POST">
                @csrf
                <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
                    <div class="md:col-span-5">
                        <label for="nome_categoria">Nome</label>
                        <input type="text" name="nome_produto" required maxlength="150" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="" />
                    </div>

                    <div class="md:col-span-5">
                        <label for="id_categoria_planejamento">Categoria</label>
                        <select required type="text" name="id_categoria_planejamento" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="">
                            <option disabled value selected></option>
                            @foreach ($categorias as $categoria)
                            <option value="{{ $categoria->id_categoria_planejamento }}">{{$categoria->nome_categoria}}</option>
                            @endforeach
                    ...
                    <div class="md:col-span-5">
                        ...
                        <input  name="valor_produto" pattern="(0\.((0[1-9]{1})|([1-9]{1}([0-9]{1})?)))|(([1-9]+[0-9]*)(\.([0-9]{1,2}))?)" required class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="" placeholder="" />
                    </div>
